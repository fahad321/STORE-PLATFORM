...
# Install AEM Content Package to Publish Instance
...
echo "📦 Installing AEM Content Package to Publish Instance..."
echo ""
...
AEM_URL="http://${AEM_PUBLISH_HOST}:${AEM_PUBLISH_PORT}"
PACKMGR_URL="${AEM_URL}/crx/packmgr/service.jsp"

echo "🔍 Checking AEM Publish instance at $AEM_URL..."

if ! curl -s -f -u "${AEM_USER}:${AEM_PASSWORD}" "$AEM_URL/system/console/bundles.json" > /dev/null 2>&1; then
    echo -e "${RED}❌ Cannot connect to AEM Publish at $AEM_URL${NC}"
    echo "   Make sure AEM SDK Publish instance is running"
    echo "   You can start it with: java -jar aem-sdk-quickstart-publish-*.jar"
    exit 1
fi

echo -e "${GREEN}✅ AEM Publish instance is reachable${NC}"
echo ""
...
echo "📤 Uploading and installing package to Publish instance..."
