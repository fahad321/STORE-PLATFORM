...
AEM_HOST="${AEM_AUTHOR_HOST:-localhost}"
AEM_PORT="${AEM_AUTHOR_PORT:-4502}"
AEM_USER="${AEM_AUTHOR_USER:-admin}"
AEM_PASSWORD="${AEM_AUTHOR_PASSWORD:-admin}"
...
delete_legacy() {
  ...
  local code
  code=$(curl -sS -o /tmp/aem-author-delete.txt -w "%{http_code}" -u "${AEM_USER}:${AEM_PASSWORD}" -X POST -F ":operation=delete" "$url" || echo "000")
  if [[ "$code" == "404" ]]; then
  ...
  if [[ "$code" != "200" && "$code" != "204" ]]; then
    ...
    cat /tmp/aem-author-delete.txt >&2 || true
    exit 1
  fi
...
install_package() {
  local pkg="$1"
  echo "Uploading package to Author: $AEM_URL"
  curl -fsS -u "${AEM_USER}:${AEM_PASSWORD}" \
    -F "file=@${pkg}" \
    -F "name=${PACKAGE_NAME}" \
    -F "force=true" \
    -F "install=true" \
    "${AEM_URL}/crx/packmgr/service.jsp" > /tmp/aem-author-install.xml

  if ! grep -q "<status code=\"200\">" /tmp/aem-author-install.xml; then
    echo "ERROR: Author install failed. Response:" >&2
    cat /tmp/aem-author-install.xml >&2
    exit 1
  fi

  echo "Author install OK"
}
...
ensure_content_fragment() {
  ...
  echo "Author content fragment seeded"
}
...
ensure_persisted_query() {
  ...
  code=$(curl -sS -o /tmp/aem-author-persist.txt -w "%{http_code}" \
  ...
  if [[ "$code" != "200" && "$code" != "201" ]]; then
    ...
    cat /tmp/aem-author-persist.txt >&2 || true
    echo "" >&2
    echo "Manual fallback (SDK UI) required to persist the query:" >&2
    echo "1) Open http://localhost:4502/aem/graphiql.html" >&2
  ...
  echo "Author persisted query ensured"
}
...
ensure_graphql_endpoint() {
  ...
  curl -fsS -u "${AEM_USER}:${AEM_PASSWORD}" \
    ...
    -F "enabled=true" \
    "$endpoint" > /dev/null
  echo "Author GraphQL endpoint ensured"
}
...
rm -f /tmp/aem-author-install.xml /tmp/aem-author-delete.txt /tmp/aem-author-persist.txt
